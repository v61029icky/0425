      ...
      this.update =function(){
        ...
        if (p5j.map.getBounds().contains(ll)){
            ...
            if (this.layerGroup.open){ // 若本layerGroup 打開
            	...
            	p5j.ellipse(p5j.mouseX, p5j.mouseY, 40, 40);

            	let d = p5j.dist(pix.x, pix.y, p5j.mouseX, p5j.mouseY);

            	if ( d < 20) 
            	{
                   p5j.line(pix.x, pix.y, p5j.mouseX, p5j.mouseY);
                   p5j.stroke(233, 139, 42);
                   p5j.fill(233, 139, 42, 95);
            	}
            	else
            	{
            		p5j.stroke(106, 76, 156);
            		p5j.fill(106, 76, 156, 10);
            	}
